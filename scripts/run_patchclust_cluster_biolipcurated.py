...
for folder in next(os.walk('.'))[1]:
    ...
    frank_dir = f"{base_path}/frankPEPstein_{pep_size}/"
    if os.path.isdir(base_path):
        if not os.path.exists(pep_tsv):
            ...
            if len([x for x in os.listdir(".") if "patch_file" in x]) > 1:
                ...
                os.system(f'python3 /work/joagutierrez/scripts/patch_clustering_V8.7_cluster.py -w {pep_size} -t 32')
                os.chdir(f"frankPEPstein_{pep_size}")
                os.system(f'cp ../../../receptor.pdb .')
                print('python3 ~/scripts/frankVINA_V3_cluster.py receptor.pdb 32 1')
                os.system(f'python3 /work/joagutierrez/scripts/frankVINA_V3_cluster.py receptor.pdb 32 1')
